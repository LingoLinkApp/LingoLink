...
export abstract class StorageService {
	...
	public static async deleteFromLocalStorage(key: string): Promise<void> {
		try {
			await AsyncStorageNative.removeItem(key);
		} catch (error) {
			if (error instanceof Error) {
				console.error(`Could not delete key ${key} from local storage: ${error.message}`);
				throw new Error(`Could not delete key ${key} from local storage: ${error.message}`);
			}
		}
	}
}
